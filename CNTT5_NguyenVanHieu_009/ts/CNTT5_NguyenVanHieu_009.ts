...
class CourseManager{
    ...
    createUser(name: string, email: string, phone: string): void{
        const user:User = new User(Date.now().toString(), name, email, phone);
        this.users.push(user);
    }
    createNewDiscount(discountCode: string, discountValue: number): void{
        const discount: DiscountCode = { code:discountCode, value:discountValue};
    ...
    handleBuyCourse(userId: string, courseId: string): string{
        ...
        if(!course) return "Khoá học không tồn tại.";
...
12.Thoát trương trình.
    `);
    let choice:string = prompt("Mời nhập vào lựa chọn của bạn:");
    switch (choice) {
        case`1`:
            courseManager.createUser(prompt("Nhập tên người dùng:"), prompt("Nhập email:"), prompt("Nhập số điện thoại:"));
            break;
        case`2`:
            const courseType:string = prompt("Nhập kiểu khoá học:");
